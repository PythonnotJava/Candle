...
const candle_int MAX_SIZE = 1024;
const candle_int HEX_VAL = 255;
typedef candle_int Int32;
/* type alias IntOrFloat */
...
static void candle_init(void) {
    ...
    candle_bool active = 1;
    candle_int* maybe = NULL;
    candle_double result = (((x + 10) * 2) - (y / 3));
    candle_int mod_val = (100 % 7);
    candle_bool check = (((x > 5) && (y < 10)) || !flag);
    candle_int bits = (255 & 15);
    candle_int shifted = (bits | 240);
    x = (x + 1);
    x += 10;
    x -= 5;
    x *= 2;
    x /= 3;
    CandleList* numbers = candle_list_new(5, (candle_int)(1), (candle_int)(2), (candle_int)(3), (candle_int)(4), (candle_int)(5));
    __auto_type first = candle_index(numbers, 0);
    CandleMap* config = candle_map_new(2, candle_str("host"), candle_str("localhost"), candle_str("port"), 8080);
