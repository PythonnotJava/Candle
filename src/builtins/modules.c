...
// random() → [0,1) 的 double；random(n) → [0,n) 的 int
static Value m_random(int argc, Value *argv) {
    ensure_seed();
    double r = (double)rand() / ((double)RAND_MAX + 1.0);
    if (argc >= 1) {
        int64_t n = arg_i(argc, argv, 0);
        if (n <= 0) return v_int(0);
        return v_int((int64_t)(r * (double)n));
    }
    return v_double(r);
}
// randomRange(lo, hi) → [lo, hi) 的 int
static Value m_random_range(int argc, Value *argv) {
    ensure_seed();
    int64_t lo = arg_i(argc, argv, 0), hi = arg_i(argc, argv, 1);
    if (hi <= lo) return v_int(lo);
    double r = (double)rand() / ((double)RAND_MAX + 1.0);
    return v_int(lo + (int64_t)(r * (double)(hi - lo)));
}
static Value m_seed(int argc, Value *argv) {
    srand((unsigned)arg_i(argc, argv, 0));
    g_rand_seeded = 1;
    return v_null();
}
...
static Value build_math(void) {
    ...
    VMap *m = mod.as.map;
    // 幂 / 指数
    v_map_set(m, "sqrt",  v_native(m_sqrt));
    v_map_set(m, "cbrt",  v_native(m_cbrt));
    v_map_set(m, "pow",   v_native(m_pow));
    v_map_set(m, "exp",   v_native(m_exp));
    v_map_set(m, "hypot", v_native(m_hypot));
    // 对数
    v_map_set(m, "log",   v_native(m_log));
    v_map_set(m, "log2",  v_native(m_log2));
    v_map_set(m, "log10", v_native(m_log10));
    // 三角
    v_map_set(m, "sin",   v_native(m_sin));
    v_map_set(m, "cos",   v_native(m_cos));
    v_map_set(m, "tan",   v_native(m_tan));
    v_map_set(m, "asin",  v_native(m_asin));
    v_map_set(m, "acos",  v_native(m_acos));
    v_map_set(m, "atan",  v_native(m_atan));
    v_map_set(m, "atan2", v_native(m_atan2));
    // 双曲
    v_map_set(m, "sinh",  v_native(m_sinh));
    v_map_set(m, "cosh",  v_native(m_cosh));
    v_map_set(m, "tanh",  v_native(m_tanh));
    // 取整
    v_map_set(m, "floor", v_native(m_floor));
    v_map_set(m, "ceil",  v_native(m_ceil));
    v_map_set(m, "round", v_native(m_round));
    v_map_set(m, "trunc", v_native(m_trunc));
    // 数值
    v_map_set(m, "abs",   v_native(m_abs));
    v_map_set(m, "sign",  v_native(m_sign));
    v_map_set(m, "min",   v_native(m_min));
    v_map_set(m, "max",   v_native(m_max));
    // 角度
    v_map_set(m, "radians", v_native(m_radians));
    v_map_set(m, "degrees", v_native(m_degrees));
    // 随机
    v_map_set(m, "random",      v_native(m_random));
    v_map_set(m, "randomRange", v_native(m_random_range));
    v_map_set(m, "seed",        v_native(m_seed));
    // 常量
    v_map_set(m, "PI",      v_double(3.14159265358979323846));
    v_map_set(m, "E",       v_double(2.71828182845904523536));
    v_map_set(m, "SQRT2",   v_double(1.41421356237309504880));
    v_map_set(m, "SQRT1_2", v_double(0.70710678118654752440));
    v_map_set(m, "LN2",     v_double(0.69314718055994530942));
    v_map_set(m, "LN10",    v_double(2.30258509299404568402));
    return mod;
}
...
static Value io_writeFile(int argc, Value *argv) {
    ...
    char *content = v_to_string(argv[1]);
    fputs(content, f);
    free(content);
    fclose(f);
    return v_bool(1);
...
static Value build_io(void) {
    Value mod = v_map();
    VMap *m = mod.as.map;
    v_map_set(m, "readline",  v_native(io_readline));
    v_map_set(m, "write",     v_native(io_write));
    v_map_set(m, "writeln",   v_native(io_writeln));
    v_map_set(m, "readFile",  v_native(io_readFile));
    v_map_set(m, "writeFile", v_native(io_writeFile));
    return mod;
}
