...
// MSVC forward declarations (headers may be suppressed by preprocessor issues)

#ifdef _MSC_VER

extern void builtins_register(Env *globals);

extern Value module_load(const char *path);

extern Value ffi_dll_load(const char *dotted_path, const char *alias_name);

extern Value ffi_call(const char *sym, const char *ret_type, int argc, Value *argv);
...
// ?? Reflect: ????? (target type name ? method decl) ??????????????????????

typedef struct {

    char *target;       // ??????"Vector" / "int" / "string" ...
...
static int check_private_caller(Env *env, AstNode *class_node) {
    Value *self = env_lookup(env, "self");
    if (!self || self->type != V_OBJECT) return 1;
    AstNode *cn = self->as.obj->class_node;
    while (cn) {
        if (cn == class_node) return 0;
        cn = find_class(cn->as.class_decl.parent);
    }
    return 1;
}

static int check_private_access(Env *env, AstNode *class_node, const char *member_name) {
    AstNode *cn = class_node;
    int is_priv = 0;
    while (cn) {
        for (int i = 0; i < cn->as.class_decl.members.count; i++) {
            AstNode *memb = cn->as.class_decl.members.items[i];
            const char *n = NULL;
            if (memb->type == NODE_FIELD_DECL) n = memb->as.var.name;
            else if (memb->type == NODE_METHOD_DECL) n = memb->as.func.name;
            if (n && strcmp(n, member_name) == 0) {
                if (memb->type == NODE_FIELD_DECL && memb->as.var.is_private) is_priv = 1;
                if (memb->type == NODE_METHOD_DECL && memb->as.func.is_private) is_priv = 1;
                break;
            }
        }
        if (is_priv) break;
    ...
    if (!is_priv) return 0;
    return check_private_caller(env, class_node);
}

static void throw_private_error(Interp *I, const char *member_name) {
    I->exc_active = 1;
    AstNode *cls = find_class("PrivateException");
    if (cls) {
        Value exc; exc.type = V_OBJECT;
        exc.as.obj = GC_CALLOC(1, sizeof(VObject));
        exc.as.obj->class_node = cls;
                Value mp = v_map(); exc.as.obj->fields = mp.as.map;
        char msg[256];
        snprintf(msg, sizeof(msg), "private member '%s' accessed externally", member_name);
        v_map_set(exc.as.obj->fields, "message", v_string(msg));
        I->exc_value = exc;
    }
}
static void throw_error(Interp *I, AstNode *node, const char *cls_name, const char *msg) {
    (void)node;
    I->exc_active = 1;
    AstNode *cls = find_class(cls_name);
    if (cls) {
        Value exc; exc.type = V_OBJECT;
        exc.as.obj = GC_CALLOC(1, sizeof(VObject));
        exc.as.obj->class_node = cls;
                Value mp = v_map(); exc.as.obj->fields = mp.as.map;
        if (msg) {
            Value msgv = v_string(msg);
            v_map_set(exc.as.obj->fields, "message", msgv);
        }
        I->exc_value = exc;
    }
}
...
// ?????alias Int64 = int????????????????????(V_MAP)?

// ?? map ???? + native ??????? int/double/bool/string ??????



// int.parse / Int64.parser?"435" ? 435??? bool/double ????????
...
static Value make_type_handle(const char *type_name) {
    ...
    if (parse) {
        ...
        v_map_set(m, "parse",  v_native(parse));   // Dart ?? int.parse(...)

        v_map_set(m, "parser", v_native(parse));   // ????? parser ??
...
static Value string_method(Value s, const char *name, int argc, Value *argv) {
    ...
    if (strcmp(name, "padLeft") == 0 || strcmp(name, "padRight") == 0) {
        ...
        size_t total = slen + (width - slen) * plen;  // ??
...
static Value bin_arith(Interp *I, AstNode *node, TokenType op, Value a, Value b) {
    ...
    switch (op) {
        ...
        case TK_SLASH:
            ...
            if ((is_d ? (db == 0.0) : (ib == 0))) { throw_error(I, node, "ArithmeticException", "division by zero"); return v_null(); }

            return is_d ? v_double(da/db) : v_int(ia/ib);

        case TK_MOD:

            if (ib == 0) { throw_error(I, node, "ArithmeticException", "modulo by zero"); return v_null(); }
...
static Value eval(Interp *I, Env *env, AstNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_BINARY: {
            ...
            // @operator overloading for V_OBJECT
            ...
            if (a.type == V_OBJECT) {
                ...
                switch (op) {
                    ...
                    case TK_PLUS:  om = "__add__"; break;

                    case TK_MINUS: om = "__sub__"; break;

                    case TK_STAR:  om = "__mul__"; break;

                    case TK_SLASH: om = "__div__"; break;

                    case TK_MOD:   om = "__mod__"; break;

                    case TK_EQ:    om = "__eq__";  break;

                    case TK_NEQ:   om = "__ne__";  break;

                    case TK_LT:    om = "__lt__";  break;

                    case TK_LTE:   om = "__le__";  break;

                    case TK_GT:    om = "__gt__";  break;

                    case TK_GTE:   om = "__ge__";  break;
                ...
                if (om) {
                    ...
                    AstNode *opm = reflect_lookup(reflect_type_of(a), om);

                    if (opm) {

                        Value argv[1] = {b};

                        return reflect_invoke(I, env, opm, a, 1, argv);
        ...
        case NODE_CALL: {
            ...
            const char *static_call_class = NULL;  // ????????
            ...
            if (is_member_call) {
                ...
                if (receiver.type == V_MAP) {
                    ...
                    if (fld.type == V_NATIVE || fld.type == V_FN || fld.type == V_LAMBDA) {
                        ...
                        // ??? Map<K,V> ???????containsKey/keys/values/...?
                ...
                if (receiver.type == V_OBJECT) {
                    ...
                    if (mth && mth->as.func.is_private && check_private_caller(env, receiver.as.obj->class_node) == 1) {
                        throw_private_error(I, method_name);
                        return v_null();
                    }



                    if (mth && mth->as.func.body == NULL) {
                    ...
                    if (mth) {
                        ...
                        if (fld.type == V_FN || fld.type == V_LAMBDA || fld.type == V_NATIVE) {
                            ...
                            if (rm) {
                                ...
                                if (rm->annotation && strcmp(rm->annotation, "deprecated") == 0) {

                                    fprintf(stderr, "warning: %s() is deprecated", method_name);

                                    if (rm->annotation_arg) fprintf(stderr, ": %s", rm->annotation_arg);
        ...
        case NODE_MEMBER: {
            ...
            if (obj.type == V_OBJECT) {
                if (check_private_access(env, obj.as.obj->class_node, node->as.member.name)) {
                    throw_private_error(I, node->as.member.name);
                    return v_null();
                }
            ...
            // ????????list/tuple/string ? first/last/length/isEmpty ?
        ...
        case NODE_THROW: {
            ...
            if (v && v->type == NODE_CALL && v->as.call.callee->type == NODE_IDENT) {
                cn = v->as.call.callee->as.ident.name;
            ...
            // Known exception class: create V_MAP with type+message
            if (find_class(cn)) {
                Value exc = v_map();
                v_map_set(exc.as.map, "type", v_string(cn));
                if (v->as.call.args.count > 0) {
                    Value arg = eval(I, env, v->as.call.args.items[0]);
                    v_map_set(exc.as.map, "message", arg);
                }
                I->exc_value = exc;
                I->exc_active = 1;
                return v_null();
        ...
        case NODE_LOAD: {
            ...
            if (mod.type == V_MAP && mod.as.map) {

                VMap *ns = mod.as.map;

                for (int _j = 0; _j < ns->len; _j++) {

                    if (ns->vals[_j].type == V_CLASS)

                        env_define(env, ns->keys[_j], ns->vals[_j]);
        ...
        case NODE_TRY: {
            ...
            if (I->exc_active && node->as.try_stmt.catch_bodies.count > 0) {
                ...
                for (int i = 0; i < node->as.try_stmt.catch_bodies.count; i++) {
                    ...
                    if (ct == NULL) { chosen = i; break; }           // ? catch ??

                    if (exc_type && strcmp(ct->as.ident.name, exc_type) == 0) { chosen = i; break; }
                    if (exc_type && chosen < 0) {
                        AstNode *cls = find_class(exc_type);
                        while (cls) {
                            if (strcmp(ct->as.ident.name, cls->as.class_decl.name) == 0) { chosen = i; break; }
                            cls = find_class(cls->as.class_decl.parent);
                        }
                    }
...
static Value load_candle_module(Interp *I, const char *dotted_path) {
    ...
    Env *modenv = env_new(g_class_env);  // ?????? + ???
...
int interp_run(AstNode *program, const char *filename) {
    ...
    // ?? Object ???? (toString / hashCode / type / equals) ?????????

    {   // toString(): self ? "<Instance of ClassName>"

        AstNode *m = ast_new(NODE_METHOD_DECL, 0, 0);

        m->as.func.name = strdup("toString");

        m->as.func.return_type = strdup("string");

        m->as.func.is_static = 0;

        node_list_init(&m->as.func.params);

        AstNode *body = ast_new(NODE_BLOCK, 0, 0);

        node_list_init(&body->as.program.stmts);

        AstNode *ret = ast_new(NODE_RETURN, 0, 0);

        AstNode *call = ast_new(NODE_CALL, 0, 0); call->is_ctor_call = 0;

        AstNode *fn = ast_new(NODE_IDENT, 0, 0); fn->as.ident.name = strdup("__object_type_name");

        call->as.call.callee = fn; node_list_init(&call->as.call.args);

        AstNode *s = ast_new(NODE_IDENT, 0, 0); s->as.ident.name = strdup("self");

        node_list_push(&call->as.call.args, s);

        ret->as.return_stmt.value = call;

        node_list_push(&body->as.program.stmts, ret);

        m->as.func.body = body;

        reflect_register("Object", m);
    ...
    {   // hashCode(): self ? ????
        ...
        m->as.func.name = strdup("hashCode"); m->as.func.return_type = strdup("int");

        m->as.func.is_static = 0; node_list_init(&m->as.func.params);

        AstNode *body = ast_new(NODE_BLOCK, 0, 0); node_list_init(&body->as.program.stmts);

        AstNode *ret = ast_new(NODE_RETURN, 0, 0);

        AstNode *call = ast_new(NODE_CALL, 0, 0); call->is_ctor_call = 0;

        AstNode *fn = ast_new(NODE_IDENT, 0, 0); fn->as.ident.name = strdup("__object_hash");
    ...
    {   // type(): self ? ??
        ...
        m->as.func.name = strdup("type"); m->as.func.return_type = strdup("string");
    ...
    {   // equals(other): identity check
        ...
        m->as.func.name = strdup("equals"); m->as.func.return_type = strdup("bool");

        m->as.func.is_static = 0;

        node_list_init(&m->as.func.params);

        AstNode *p = ast_new(NODE_VAR_DECL, 0, 0);

        p->as.var.type_name = strdup("Object"); p->as.var.name = strdup("other"); p->as.var.init = NULL;

        node_list_push(&m->as.func.params, p);
        ...
        AstNode *fn = ast_new(NODE_IDENT, 0, 0); fn->as.ident.name = strdup("__object_identical");
        ...
        AstNode *o = ast_new(NODE_IDENT, 0, 0); o->as.ident.name = strdup("other");

        node_list_push(&call->as.call.args, o);
...
// ?? ??????????? + builtin extras ??????????????????????
    ...
    { const char *auto_mods[] = {

        "library.std.Int", "library.std.Double", "library.std.Bool",

        "library.std.String", "library.std.List", "library.std.Map",

        "library.std.builtins_extras",

        "library.std.encoding",
      ...
      for (int ai = 0; ai < 9; ai++)
          ...
          load_candle_module(&I, auto_mods[ai]);



      Value extras = load_candle_module(&I, "library.std.builtins_extras");

      if (extras.type == V_MAP && extras.as.map)

          for (int ei = 0; ei < extras.as.map->len; ei++)

              env_define(globals, extras.as.map->keys[ei], extras.as.map->vals[ei]);



      // ?? Encoding: inject module functions + global constants ??

      Value enc_mod = module_load("std.encoding");

      if (enc_mod.type == V_NULL)

          enc_mod = load_candle_module(&I, "library.std.encoding");

      if (enc_mod.type == V_MAP && enc_mod.as.map) {

          for (int ei = 0; ei < enc_mod.as.map->len; ei++)

              env_define(globals, enc_mod.as.map->keys[ei], enc_mod.as.map->vals[ei]);

      }

      env_define(globals, "utf8",   v_string("UTF-8"));

      env_define(globals, "ascii",  v_string("ASCII"));

      env_define(globals, "latin1", v_string("Latin-1"));
