        ...
        if (check(p, TK_RPAREN)) {
            ...
            for (;;) {
                ...
                if (is_type_token(p->current.type)) {
                    ...
                    advance(p);                         // type ? ?? ident

                    if (check(p, TK_QUESTION)) advance(p);

                    if (check(p, TK_IDENT)) advance(p); // "type name" ??
...
static char *parse_type_name(Parser *p) {
    ...
    // ???????BaseType Function(TypeList)  ?  ??????? "Function"
    ...
    if (!check(p, TK_RPAREN)) {
        ...
        do {
            ...
            if (check(p, TK_IDENT)) {
                ...
                // function pointer type: int fn(int, double) -> skip inner params

                if (check(p, TK_LPAREN)) {

                    advance(p); // consume (

                    param->as.var.is_func_type = 1;

                    node_list_init(&param->as.var.func_type_params);

                    if (!check(p, TK_RPAREN)) {

                        do {

                            AstNode *fp = ast_new(NODE_VAR_DECL, p->current.line, p->current.column);

                            fp->as.var.type_name = parse_type_name(p);

                            fp->as.var.name = strdup("_");

                            node_list_push(&param->as.var.func_type_params, fp);

                        } while (match(p, TK_COMMA));

                    }

                    consume(p, TK_RPAREN, "expected ')' after function type params");
...
static AstNode *parse_class_decl(Parser *p) {
    ...
    if (match(p, TK_INHERIT)) {
        ...
        if (strcmp(node->as.class_decl.name, "Object") != 0)

            node->as.class_decl.parent = strdup("Object");
    ...
    while (!check(p, TK_RBRACE) && !check(p, TK_EOF)) {
        ...
        char *annotation_arg = NULL;

        if (match(p, TK_AT)) { int alen = p->current.length; annotation = malloc(alen+1); memcpy(annotation, p->current.start, alen); annotation[alen]=0; advance(p); if (match(p, TK_LPAREN) && check(p, TK_STRING_LIT)) { annotation_arg = strdup(p->current.start + 1); annotation_arg[strlen(annotation_arg)-1] = 0; advance(p); consume(p, TK_RPAREN, "expected ')' after annotation arg"); } }
        ...
        if (is_type_token(p->current.type)) {
            ...
            method->annotation_arg = annotation_arg;
...
static AstNode *parse_reflect_stmt(Parser *p) {
    ...
    while (!check(p, TK_RBRACE) && !check(p, TK_EOF)) {
        ...
        // ????static / private / public / final

            annotation = NULL; annotation_arg = NULL; if (match(p, TK_AT)) { int alen = p->current.length; annotation = malloc(alen+1); memcpy(annotation, p->current.start, alen); annotation[alen]=0; advance(p); if (match(p, TK_LPAREN) && check(p, TK_STRING_LIT)) { annotation_arg = strdup(p->current.start + 1); annotation_arg[strlen(annotation_arg)-1] = 0; advance(p); consume(p, TK_RPAREN, "expected ')' after annotation arg"); } }
